...
import com.project2.database.classes.LinksDbAccess;
...
@WebServlet(name = "shortnerServlet", urlPatterns = { "/shortner" }, loadOnStartup = 1)
...
public class ShortnerServlet extends HttpServlet {
	...
	private void showUrlForm(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		request.getRequestDispatcher("/WEB-INF/jsp/view/urlForm.jsp").forward(request, response);
	}

	private void listUrls(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		...
		LinksDbAccess linksdao = (LinksDbAccess) context.getBean("LinksDbAccess");
		User user = dao.getUserID(Databaseusername);
		Links link = (Links) context.getBean("Links");
		List<Links> linkbyid= (List<Links>) linksdao.getLinksByUser(user.getId());
		System.out.println("From shortener servlet:" +linkbyid);
		//System.out.println("SHORT:"+linkbyid.getShortUrl()+"LONG:"+linkbyid.getLongUrl()+"CLICKS:"+linkbyid.getClicks());
		//linkdao.getLinksByUser(user.getId());
		
		
		request.setAttribute("listoflinks", linkbyid);
	...
	private void createUrl(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		...
		/*
		 * Iterator it = shortnerDatabase.entrySet().iterator(); while
		 * (it.hasNext()) { Map.Entry pair = (Map.Entry)it.next();
		 * 
		 * UserUrl u = (UserUrl) pair.getValue();
		 * 
		 * System.out.println(pair.getKey() + " = " + u.getlongUrl() + ", " +
		 * u.getshortUrl() + ", " + u.getclicks());
		 * System.out.println(pair.getValue() ); // it.remove(); // avoids a
