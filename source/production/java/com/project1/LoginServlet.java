...
public class LoginServlet extends HttpServlet
{
	...
	private void login_check(HttpServletRequest request,
...
throws ServletException, IOException
{
        ...
        /*User userDetails = (User) ctx.getBean("User");
   	 	userDetails.setUsername(loginUsername);
   	 	userDetails.setPassword(loginPassword); */  
   	 	
   	 	UserDbAccess loginDao = (UserDbAccess) ctx.getBean("UserDbAccess"); 
   	 	User user = loginDao.getUser(loginUsername, loginPassword);
   	 	//System.out.println("Hi these values are returned from database: "+user.getUsername());
   	 ...
   	 else
     {
    	...
    	/*if(loginUsername == null || loginPassword == null ||
                 !LoginServlet.userDatabase.containsKey(loginUsername) ||
                 !loginPassword.equals(LoginServlet.userDatabase.get(loginUsername)))
         {
             request.setAttribute("loginFailed", true);
             request.getRequestDispatcher("/WEB-INF/jsp/view/login.jsp")
                    .forward(request, response);
    ...
    private void signinForm (HttpServletRequest request,
            HttpServletResponse response)
throws ServletException, IOException
{
    	 request.getRequestDispatcher("/WEB-INF/jsp/view/signinForm.jsp")
         .forward(request, response);
}
    
    private void signup_add (HttpServletRequest request,
            HttpServletResponse response)
throws ServletException, IOException
{
    	...
    	System.out.println("uname : "+signin_username+"  pass:  "+signin_password);
     	...
     	dao.insertUser(user);
    	 //userDatabase.put(signin_username, signin_password);
    	 //System.out.println(signin_username+signin_password);
    	 response.sendRedirect("login");
    	 
    	 
}
    
    private void show_long_url (HttpServletRequest request,
            HttpServletResponse response)
throws ServletException, IOException
{
    ...
    /*
    private void formlong_url (HttpServletRequest request,
            HttpServletResponse response)
throws ServletException, IOException
{
    	request.getRequestDispatcher("/WEB-INF/jsp/view/publicForm.jsp")
        .forward(request, response);
    	//String public_short=request.getParameter("ip_shorturl");
    	//if(ShortnerServlet.shortnerDatabase.containsKey(public_short)){
    		//System.out.println("trials:"+ShortnerServlet.shortnerDatabase.get(public_short));
    	//}
    	//System.out.println("Trials"+ShortnerServlet.shortnerDatabase);
}*/
}
